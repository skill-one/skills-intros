...
[skill-one/skills-sh-mirror](https://github.com/skill-one/skills-sh-mirror): a queryable index plus
eight LLM-written angles per skill, generated from its `SKILL.md` and published as self-contained
snapshots.
...
The index folds in only the two angles you filter on; all eight are per-skill files, each with its own
schema:
...
apart, so re-drawing one costs no LLM call. Full detail: [DEVELOPING.md](DEVELOPING.md).
...
filter it: `jq -r 'select(.domain.domain == "设计多媒体") | [.id, .persona.role] | @tsv'`. The whole
...
jq -r --slurpfile up up.jsonl '($up | map({(.id): .installs}) | add) as $i | [.id, $i[.id], .domain.domain, .persona.role] | @tsv' mine.jsonl
```
