...
- **File-based resume.** Each prompt's output is its own `<prompt_id>.json` (plus a
  ...
  `invalidate --stale` compares against; neither `run` nor `sync` re-checks hashes. Resume
...
`hashes.json`, i.e. it counts as new again. `invalidate --stale` selects the skills whose
upstream hash changed (or that vanished from the snapshot) and runs the same `invalidate`.
