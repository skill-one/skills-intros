...
- **File-based resume.** Each prompt's output is its own `<prompt_id>.json`, committed
  (json+md) right after generation — both the output and the cache: present and
...
always rerun; their dependencies are inputs, so they reuse their stored per-prompt json and
are regenerated only when missing or schema-invalid. `--force` applies to the requested
prompts only, not to the dependencies pulled in by the closure. Prompts outside the closure
are not touched, so nothing outside the selection is ever regenerated by accident.
...
├── outputs.py       # per-prompt json+md output writing and rendering
