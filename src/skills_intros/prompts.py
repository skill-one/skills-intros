...
`_system.md` holds the shared system prompt. Templates are rendered with jinja2;
the DAG is ordered with the stdlib graphlib.TopologicalSorter. The prompts
directory defaults to `./prompts` and can be overridden with
SKILLS_INTROS_PROMPTS_DIR.
"""
...
# Template context: `skill` (SkillRecord), `skill_md` (its source text),
# `deps` (dict of prompt_id -> parsed output of upstream prompts),
# `domain_taxonomy` (rendered '- name: description' lines from models.Domain).
...
@dataclass(frozen=True)
class PromptSet:
    """All prompts loaded from one directory, plus the shared system prompt."""
...
def load_prompt_set(directory: Path) -> PromptSet:
    ...
    return PromptSet(
        directory=directory,
        system_prompt=(directory / "_system.md").read_text(encoding="utf-8").strip(),
        by_id=by_id,
    )
...
def render_user_prompt(spec: PromptSpec, skill: Any, deps: dict[str, Any]) -> str:
    """Render the user prompt for one spec; `deps` maps prompt_id -> parsed output."""
    return _env.from_string(spec.template).render(
        skill=skill, skill_md=skill.skill_md, deps=deps,
        domain_taxonomy=Domain.taxonomy_text(),
    )
