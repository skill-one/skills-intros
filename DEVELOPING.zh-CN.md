...
- **配图 = 配方 + 渲染** —— `cover` 是一个普通 prompt，因此继承了 DAG、缓存和 `invalidate`；
  `images.py` 在配方的主体之上补上取景与每个 `Domain` 的画风。配图的缓存就是文件本身是否存在，因为
  接口返回的是一个一小时后就过期的 url——存下来的是字节，url 从不保存——而重渲染不花 LLM 调用。
...
`skills.jsonl` 除名。闭包只向依赖走，从不向依赖它的人走：失效 `persona` 之后，`cover` 配方（以及由它
渲染出的配图）仍描述着那幅旧的职业画像——真要连带重做就把它说出来：`invalidate --prompts persona,cover`。
...
把 prompt id 加进 `outputs.py` 的 `AGGREGATED_PROMPTS`，它就会折进每一行 `skills.jsonl`；若有非 json
的产物，在同一个文件里的 `PROMPT_ASSETS` 登记它的后缀——`cover` 就是这样持有 `cover.png` 的，也正因
如此，失效这个 prompt 时会连同配方一起丢弃渲染出的产物。
