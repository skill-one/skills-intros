...
```bash
skills-intros run --limit 0           # 所有缺 prompt 的 skill
skills-intros run --prompts tagline   # 只生成这一个 prompt
skills-intros run --limit 5 --dry-run # 离线冒烟测试（假 LLM，无 API 调用）
skills-intros run --limit 5 --debug   # 打印渲染后的 prompt 到 stderr
skills-intros run --verbose           # DEBUG 日志
```
...
skills-intros invalidate --stale                   # 上游内容变化（或消失）的 skill
...
优先级从高到低：`SKILLS_INTROS_*` 环境变量 → 本地 `.env` → 内置默认值。
...
| `SKILLS_INTROS_PROMPTS_DIR` | `prompts` | prompt markdown 目录（含 `_system.md`） |
