...
[agent skills](https://www.skills.sh) 生成的多角度中文档案：一个可查询的索引，加上每个 skill 八份
由 LLM 依据它的 `SKILL.md` 写出的档案，按自包含的快照发布。
...
索引只折入你会拿去筛选的那两个角度；八个角度本身都是每个 skill 目录下的文件，各有各的结构：
...
`jq -r 'select(.domain.domain == "设计多媒体") | [.id, .persona.role] | @tsv'`。整份快照只需一个请求
